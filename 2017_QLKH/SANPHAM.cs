...
namespace _2017_QLKH
{
    public partial class SANPHAM : Form
    {
        ...
        private void SANPHAM_Load(object sender, EventArgs e)
        {
            ...
            dgv_SANPHAM.DataSource = acc.Select_Data("	SELECT MASP,TENSP,GIA,SERIAL,(SELECT TENNHACC FROM NHACUNGCAP WHERE MANCC=SP.MANCC) AS TENNCC,(SELECT TENDANHMUC FROM DANHMUC WHERE MADANHMUC=SP.MADANHMUC) AS TENDANHMUC,NGAYSANXUAT,HANSUDUNG,SOLUONG,PHANLOAI,HINHANH,GHICHU  FROM SANPHAM SP");
        ...
        {
            if (dgv_SANPHAM.SelectedRows == null)
            {
                MessageBox.Show("Hãy Chọn trên bảng dữ liệu sản phẩm");
            }
            else
            {
                Disable();
                if (e.RowIndex > dgv_SANPHAM.Rows.Count || e.RowIndex < 0)
                {
                    cleartext();
                    pictureBox_sanpham.Image = null;
                ...
                else
                {
                    ...
                    txb_ncc.Text = dgv_SANPHAM.CurrentRow.Cells["TENNCC"].Value.ToString().Trim();
        ...
        private void btn_timkiem_Click(object sender, EventArgs e)
        {
            ...
            {
                dgv_SANPHAM.DataSource = acc.Select_Data("SELECT *FROM SANPHAM WHERE MASP like '%" + txb_timkiem.Text + "%' OR TENSP like '%" + txb_timkiem.Text + "%' OR MANCC like '%" + txb_timkiem.Text + "%' OR  GIA like '%" + txb_timkiem.Text + "%' OR MADANHMUC like '%" + txb_timkiem.Text + "%' OR SERIAL like '%" + txb_timkiem.Text + "%' OR  PHANLOAI like '%" + txb_timkiem.Text + "%'");
                txb_timkiem.Clear();
                dgv_SANPHAM.ClearSelection();
        ...
        private void button6_Click_1(object sender, EventArgs e)
        {
            if (key == 1)
            {
                if (tbx_tensp.Text == "" || txb_madm.Text == "" || txb_ghichu.Text == "" || txb_ncc.Text == "" || txb_phanloai.Text == "" || txb_serial.Text == "" || txb_soluong.Value == 0)
                {
