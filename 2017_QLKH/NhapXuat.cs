...
namespace _2017_QLKH
{
    public partial class NhapXuat : Form
    {
        ...
        private void NhapXuat_Load(object sender, EventArgs e)
        {
            ...
            dgv_phieunhap.DataSource = acc.Select_Data("SELECT PN.MAPN,(SELECT TENKHO FROM KHOHANG WHERE MAKHO=PN.MAKHO) AS TENKHO,(SELECT TENNV FROM NHANVIEN WHERE MANV=PN.NVNHAP) AS TENNHANVIEN,NGAYNHAP,(SELECT TENNHACC FROM NHACUNGCAP WHERE MANCC=PN.MANCC) AS TenNhaCungCap,(SELECT TENSP FROM SANPHAM WHERE MASP=CT.MASP) AS TENSP,SOLUONG,TONGTIEN,GHICHU FROM PHIEUNHAPKHO PN INNER JOIN CHITIETPHIEUNHAP CT ON PN.MAPN=CT.MAPN");
            dgv_phieuxuat.DataSource = acc.Select_Data("SELECT PX.MAPX,(SELECT TENKHO FROM KHOHANG WHERE MAKHO=PX.MAKHO) AS TENKHO,(SELECT TENNV FROM NHANVIEN WHERE MANV=PX.NVXUAT) AS TENNHANVIEN,NGAYXUAT,(SELECT TENKH FROM KHACHHANG WHERE MAKH=PX.MAKH) AS TenKhachHang,(SELECT TENSP FROM SANPHAM WHERE MASP=CT.MASP) AS TENSP,SOLUONG,TONGTIEN,GHICHU FROM PHIEUXUAT PX INNER JOIN CHITIETPHIEUXUAT CT ON PX.MAPX=CT.MAPX");
        ...
        private void dgv_phieuxuat_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txb_manvPX.Text = dgv_phieuxuat.CurrentRow.Cells["TENNHANVIEN"].Value.ToString();
            ...
            txb_makh.Text = dgv_phieuxuat.CurrentRow.Cells["TenKhachHang"].Value.ToString();
        ...
        private void dgv_nhapSP_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txb_nvnhap.Text = dgv_phieunhap.CurrentRow.Cells["TENNHANVIEN"].Value.ToString().Trim();
            ...
            txb_mancc.Text = dgv_phieunhap.CurrentRow.Cells["TenNhaCungCap"].Value.ToString().Trim();
